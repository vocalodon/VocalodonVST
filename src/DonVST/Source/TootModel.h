...
public:
	...
	DonMediaAttachmentModel(UInt64 pId, string pType, string pPreviewUrl, string pSize) {
		this->id = pId;
		this->type = pType;
...
public:
	...
	DonTootModel(picojson::object obj, DonAccountModel* acnt) {
		...
		if (obj["reblog"].to_str() != "null") {
			...
			while (itr != boost_medias.end()) {
				...
				UInt64 m_id = stoi(boost_media["id"].to_str());
	...
	void addMediaAttachments(UInt64 pId,string pType,string pPreviewUrl,string pSize) {
